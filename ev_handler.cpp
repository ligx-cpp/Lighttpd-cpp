...
bool ev_handler::start_ev(){     
	m_base=event_base_new();
        ...
        //m_exit_event=evsignal_new(m_base,SIGPIPE,ev_handler::ev_exit,m_base);//为了解决客户端断开后导致服务器进程结束的问题, 使用如下方法忽略SIGPIPE信号
        //evsignal_add(m_exit_event,NULL);
	event_base_dispatch(m_base);
	return true;
}

/*void ev_handler::ev_exit(evutil_socket_t sig,short event,void* arg){
